...
class cvBridgeDemo():
    ...
    def show_img_cb(self,event):
		...
		cv2.moveWindow("Processed_Image", 25, 75)

		cv2.namedWindow("Processed_Image1", cv2.WINDOW_NORMAL)
		cv2.moveWindow("Processed_Image1", 500, 75)
		
		#cv2.namedWindow("Processed_Image2", cv2.WINDOW_NORMAL)
		#cv2.moveWindow("Processed_Image2", 25, 500)
    ...
    def image_callback(self, ros_image):
        ...
        #self.display_image1 = self.process_image1(frame)
	...
	kernel = np.ones((7,7), np.uint8)
    	...
    	lane_mask[0:h, w/2:w] = 0
    	lane_mask[0:h/2, 0:w] = 0
    	lane_mask[50+h/2:h, 0:w] = 0
	...
	img1=cv2.cvtColor(frame,cv2.COLOR_BGR2RGB)
    	...
    	lane_mask1  = binary_hsv(bird_view)
	kernel = np.ones((7,7), np.uint8)
    	lane_forward = cv2.morphologyEx(lane_mask1, cv2.MORPH_OPEN, kernel)    	
    	h, w= lane_forward.shape[:2]
    	search_top = 2*h/4
    	search_bot = 2*h/4+50 
    	lane_forward[0:search_top, 0:w] = 0
    	lane_forward[search_bot:h, 0:w] = 0
	lane_forward[0:h, 0:w/3] = 0
    	M = cv2.moments(lane_forward)
    	if M['m00'] > 0:
        	cxm = int(M['m10']/M['m00'])
        	cym = int(M['m01']/M['m00'])
      		self.c=cxm - 240
